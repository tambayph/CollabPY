...
isohyet_folder_path = os.path.join(folder, 'gsmapsynop_images')

def gsmapsynop_serve_latest_image(request):
    try:
...
def index(request):
    img_dir = f'{folder}/gsmapsynop_images/'
    ...
    if query is None or query.strip() == "":
        ...
        if not filtered_files:
           return render(request, 'gsmapsynop_index.html', {'files': files, 'query': query, 'not_found_message': 'File not Found'})
    ...
    return render(request, 'gsmapsynop_index.html', {'files': files, 'query': query})

def gsmapsynop_download_image(request, file_name):
    img_dir = f'{folder}/gsmapsynop_images/' # Update this path to the folder where your img files are stored
