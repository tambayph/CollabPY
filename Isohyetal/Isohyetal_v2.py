...
if response.status_code == 200:
    ...
    for station in data:
       ...
       # Check if the total value is a string that cannot be converted to float
        if station["total"] is not None and not station["total"].isdigit():
            total_value = station["total"]  # Use the original string value
        else:
            # Convert total to float and round it to one decimal place
            total_value = round(float(station["total"]), 1) if station["total"] is not None else 0
...
fig.tight_layout()
plt.savefig(f'{folder}/isohyet_images/{date}.png', bbox_inches='tight', pad_inches=0.1, dpi=1200)
...
nas_remote_path = 'wd.s.dstor.pagasa.local/wfs/Isohyetal/'
...
transfer_to_nas(f'{folder}/isohyet_images/{date}.png', f'{nas_local_dir}/{previous_day_formatted}/')
