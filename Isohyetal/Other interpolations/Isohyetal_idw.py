...
idw = scipy.interpolate.griddata((lon, lat), obs_rr, (grid_lon, grid_lat), method='linear')

# Create a GeoDataFrame for the interpolated data
interpolated_data = pd.DataFrame({
    'Longitude': grid_lon.flatten(),
    'Latitude': grid_lat.flatten(),
    'Interpolated_Rainfall': idw.flatten()
})
...
fig.tight_layout()
plt.savefig(f'{folder}/isohyet_images/{date}idw.png', bbox_inches='tight', pad_inches=0.1, dpi=1200)
plt.show()
