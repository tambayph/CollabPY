...
# Calculate pairwise spatial distances using Euclidean distance
distances = scipy.spatial.distance.cdist(np.column_stack((lon, lat)), np.column_stack((lon, lat)))

# Perform kriging interpolation on the grid
kriging = scipy.interpolate.Rbf(lon, lat, obs_rr, function='linear', smooth=0)  # Adjust smooth parameter as needed

# Evaluate kriging function on the grid
grid_rr = kriging(grid_lon, grid_lat)
...
fig.tight_layout()
plt.savefig(f'{folder}/isohyet_images/{date}krig.png', bbox_inches='tight', pad_inches=0.1, dpi=1200)
plt.show()
