...
import scipy
# Perform natural neighbor interpolation on the grid
natural_neighbor = scipy.interpolate.griddata((lon, lat), obs_rr, (grid_lon, grid_lat), method='nearest')

# Create a GeoDataFrame for the interpolated data
interpolated_data = pd.DataFrame({
    'Longitude': grid_lon.flatten(),
    'Latitude': grid_lat.flatten(),
    'Interpolated_Rainfall': natural_neighbor.flatten()
})
...
fig.tight_layout()
plt.savefig(f'{folder}/isohyet_images/{date}nni.png', bbox_inches='tight', pad_inches=0.1, dpi=1200)
plt.show()
