...
folder_path = f'{nas_local_dir}/Tropical Cyclone/Tropical Cyclone Best Track Data/PH{year}BST/'
file_pattern = '*.txt'
...
m = Basemap(projection='merc', llcrnrlon=90, llcrnrlat=-0, urcrnrlon=180, urcrnrlat=45, resolution='h')
...
def plot_dataframe_with_annotation(df, color, linewidth, label, annotation_column):
    ...
    m.plot(x, y, linestyle='-', color='black', linewidth=linewidth, label=label)
    plt.scatter(x[0], y[0], c='black', marker='o', s = 0.1)
...
# Save the plot
plt.savefig(f'{folder}/summary_images/{year}BST.png', dpi=300, bbox_inches='tight')
...
transfer_to_nas(f'{folder}/summary_images/{year}BST.png', transfertoterra)
